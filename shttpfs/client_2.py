...
import os, sys, time, json, base64, fnmatch, shutil, fcntl, errno, urlparse
import pysodium
...
def authenticate(previous_token = None):
    ...
    if headers['status'] == 'ok':
        signature = base64.b64encode(pysodium.crypto_sign_detached(headers['auth-token'].decode('utf-8'), config['private_key']))
        headers = server_connection.request("authenticate", {
        ...
        if headers['status'] == 'ok': return headers['session-token']
...
def update(session_token, testing = False):
    ...
    req_result, headers = server_connection.request("find_changed", {
        ...
        "previous-revision"    : manifest['have_revision'],
        }, {
